...
import '@yumyumswap/contract-core/contracts/interfaces/callback/IYumyumSwapFlashCallback.sol';
import '@yumyumswap/contract-core/contracts/libraries/LowGasSafeMath.sol';
...
/// @notice An example contract using the YumYumSwap flash function
contract PairFlash is IYumyumSwapFlashCallback, PeripheryPayments {
    ...
    function yumyumswapFlashCallback(
        uint256 fee0,
        uint256 fee1,
        bytes calldata data
    ) external override {
    ...
    /// @notice Calls the pools flash function with data needed in `yumyumswapFlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        IYumyumSwapPool pool = IYumyumSwapPool(PoolAddress.computeAddress(factory, poolKey));
