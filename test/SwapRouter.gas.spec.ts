import { abi as IYumyumSwapPoolABI } from '@yumyumswap/contract-core/artifacts/contracts/interfaces/IYumyumSwapPool.sol/IYumyumSwapPool.json'
import { Fixture } from 'ethereum-waffle'
import { BigNumber, constants, ContractTransaction, Wallet } from 'ethers'
import { ethers, waffle } from 'hardhat'
import { IYumyumSwapPool, IWETH9, MockTimeSwapRouter, TestERC20 } from '../typechain'
...
describe('SwapRouter gas tests', function () {
  ...
  const swapRouterFixture: Fixture<{
    ...
    pools: [IYumyumSwapPool, IYumyumSwapPool, IYumyumSwapPool]
    ...
    const pools = poolAddresses.map((poolAddress) => new ethers.Contract(poolAddress, IYumyumSwapPoolABI, wallet)) as [
      IYumyumSwapPool,
      IYumyumSwapPool,
      IYumyumSwapPool
    ]
  ...
  let pools: [IYumyumSwapPool, IYumyumSwapPool, IYumyumSwapPool]
  ...
  describe('#exactInput', () => {
    ...
    it('0 -> 1 minimal', async () => {
      const calleeFactory = await ethers.getContractFactory('TestYumyumSwapCallee')
